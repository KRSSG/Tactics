...
namespace Strategy {
	...
	TDefendARC_right::TDefendARC_right(int botID) : Tactic(botID) {
		iState = POSITION;
	}

	TDefendARC_right::~TDefendARC_right() {}

	bool TDefendARC_right::isCompleted(const BeliefState& state,const Tactic::Param& tParam) const {
		return false;
	}//isCompleted

	inline bool TDefendARC_right::isActiveTactic(void) const {
		return iState != KICK;
	}//isActiveTactic
	...
	gr_Robot_Command TDefendARC_right::execute(const BeliefState& state, const Param& tParam) {
		Vector2D<float> P1;
		Vector2D<float> P2;
		Vector2D<float> C(-HALF_FIELD_MAXX, 0.0f);
		...
		switch(iState) {
			case POSITION:
			{
	            ...
	            sParam.GoToPointP.y = sol_mid.y + delat_y; 				
    ...
    Tactic::Param TDefendARC_right::paramFromJSON(string json) {
      using namespace rapidjson;
      Tactic::Param tParam;
      return tParam;
    }

    string TDefendARC_right::paramToJSON(Tactic::Param tParam) {
      return string("");
    }
	int TDefendARC_right::primary_threat(const BeliefState& state) const {
	...
	void TDefendARC_right::inter_circle_and_line(Vector2D<float> P1, Vector2D<float> P2, Vector2D<float> C, float R, Vector2D<float>& P) const {
	...
	bool TDefendARC_right::ball_velocity_direction(const BeliefState& state) const {
...
/*
	void tDefendARC_right::secondary_threat(const BeliefState& state, std::vector<int> &vec, int pri_threat) const {
